...
void lept_value::lept_free() {
    ...
    if (this->type == LEPT_OBJECT) {
        for (size_t i = 0; i < this->u.o.size; i++) {
            free(this->u.o.m[i].k);
            this->u.o.m[i].v.lept_free();
        }
        free(this->u.o.m);
    }
    this->type = LEPT_NULL;
